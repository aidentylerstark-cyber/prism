 ...
 *  - Name filtering is applied while rebuilding the rows (the base viewer's
 *    LLScrollListCtrl has no setFilterString()/setFilterColumn()).
 *  - The "last selected item" is not restored after a list rebuild (no
 *    setLastSelectedItem() in the base scroll list).
 *  - The Firestorm toast reporting option ("Report To") was not ported.
 *  - The column configuration is stored in the RadarColumnConfig setting.
 * $/LicenseInfo$
 */
...
FSPanelRadar::~FSPanelRadar()
{
    ...
    if (mRadarColumnConfigConnection.connected())
    {
        mRadarColumnConfigConnection.disconnect();
    }
...
bool FSPanelRadar::postBuild()
{
    ...
    mRadarColumnConfigConnection = gSavedSettings.getControl("RadarColumnConfig")->getSignal()->connect(boost::bind(&FSPanelRadar::onColumnDisplayModeChanged, this));
...
void FSPanelRadar::onFilterEdit(const std::string& search_string)
{
    ...
    // Apply new filter by rebuilding the list from the current radar data.
    // (Prism: filtering happens in updateList(); the base viewer's scroll
    // list has no built-in row filtering.)
    requestUpdate();
}
...
void FSPanelRadar::updateList(const std::vector<LLSD>& entries, const LLSD& stats)
{
    ...
    for (const auto& avdata : entries)
    {
        ...
        // Prism: apply the name filter here; see onFilterEdit().
        if (!mFilterSubString.empty())
        {
            std::string name_upper = entry["name"].asString();
            LLStringUtil::toUpper(name_upper);
            if (name_upper.find(mFilterSubString) == std::string::npos)
            {
                continue;
            }
        }
    ...
    if (LLScrollListColumn* column = mRadarList->getColumn("name"); column && column->mHeader)
    {
...
{
    U32 column_config = gSavedSettings.getU32("RadarColumnConfig");
...
void FSPanelRadar::onColumnVisibilityChecked(const LLSD& userdata)
{
    ...
    gSavedSettings.setU32("RadarColumnConfig", new_value);
}
