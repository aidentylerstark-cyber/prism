 ...
 *
 * Adapted for Prism from the Firestorm Viewer sources. Differences from
 * Firestorm in this initial port:
 *  - RLVa calls go through rlvshim.h (no-op until RLVa is ported).
 *  - FSKeywords, LGGContactSets, FSAssetBlacklist (derender), the FSLSLBridge
 *    z-offset machinery and Milkshake-style toast alerts were removed.
 *  - LFSimFeatureHandler was replaced by the Second Life chat range
 *    constants (say = 20 m, shout = 100 m).
 *  - Voice power levels are derived locally from LLVoiceClient::
 *    getCurrentPower() (the base viewer has no getPowerLevel()).
 *  - Muted avatars are always listed (italicized); Firestorm's
 *    FSRadarShowMutedAndDerendered toggle was not ported.
 *  - Alert *sounds* (UISndRadar*) are deferred to a later phase.
 * $/LicenseInfo$
 */
...
// Prism: Second Life chat ranges. Firestorm queries these via
// LFSimFeatureHandler, which only matters for OpenSim grids.
constexpr F32 FS_RADAR_CHAT_RANGE_SAY = 20.f;
constexpr F32 FS_RADAR_CHAT_RANGE_SHOUT = 100.f;
...
void FSRadar::updateRadarList()
{
    ...
    const F32 chat_range_say = FS_RADAR_CHAT_RANGE_SAY;
    const F32 chat_range_shout = FS_RADAR_CHAT_RANGE_SHOUT;
    ...
    for (;pos_it != pos_end && item_it != item_end; ++pos_it, ++item_it)
    {
        ...
        // Prism: Firestorm corrects unknown z positions (coarse locations above
        // 1020 m) via its LSL bridge; without the bridge the range simply stays
        // unknown for those avatars.
        ...
        if (relation && !gRlvHandler.hasBehaviour(RLV_BHVR_SHOWNAMES))
        {
        ...
        // Prism: derived from LLVoiceClient::getCurrentPower() with Firestorm's
        // thresholds; the base viewer has no LLVoiceClient::getPowerLevel().
        if (voice_client->voiceEnabled() && voice_client->isVoiceWorking())
        {
            ...
            {
                static const F32 power_level_0 = LLVoiceClient::OVERDRIVEN_POWER_LEVEL / 3.f;
                static const F32 power_level_1 = LLVoiceClient::OVERDRIVEN_POWER_LEVEL * 2.f / 3.f;
                static const F32 power_level_2 = LLVoiceClient::OVERDRIVEN_POWER_LEVEL;

                F32 power = voice_client->getCurrentPower(avId);
                if (voice_client->getOnMuteList(avId))
                {
                    // no icon for voice-muted avatars
                }
                else if (power == 0.f && !voice_client->getIsSpeaking(avId))
                {
    ...
    // 3b: dispatch the script channel alerts. These are self-addressed
    // ScriptDialogReply messages (ObjectID == our own agent id) on a
    // user-configured channel; they only reach the user's own scripted
    // attachments and are throttled to FSRADAR_MAX_AVATARS_PER_ALERT
    // UUIDs per message.
...
// Note: currently teleportViaLocation is disrupted by enforced landing points set on a parcel.
{
    if (auto entry = getEntry(targetAv))
    {
        ...
        {
            // Prism: Firestorm applies configurable lateral/vertical offsets
            // here (FSTeleportToOffsetLateral/Vertical); we teleport straight
...
{
    // Prism: the base viewer's LLTracker has no LOCATION_AVATAR location
    // type, so track the "radar is tracking an avatar" state locally via
    // mTrackedAvatarId instead.
...
void FSRadar::updateTracking()
{
    if (auto entry = getEntry(mTrackedAvatarId))
    {
        ...
        {
            LLTracker::trackLocation(entry->mGlobalPos, entry->mName, "");
        }
    }
    else
    {
...
{
    // Prism: the base viewer has no LLAvatarActions::zoomIn()/canZoomIn();
    // zoom directly via handle_zoom_to_object() when the avatar is rezzed.
    if (gObjectList.findObject(avatar_id))
    {
        handle_zoom_to_object(avatar_id);
    }
    else
    {
        LLStringUtil::format_map_t args;
