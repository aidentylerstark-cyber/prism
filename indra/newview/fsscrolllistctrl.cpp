...
:   LLScrollListCtrl(p),
    ...
    mHandleDaDCallback()
{
    // Prism: remember the columns we were created with so callers
    // (e.g. the radar column visibility feature) can re-add them later.
...
{
    // Prism: the base viewer's LLScrollListCtrl has no public setLineHeight()
    // and mLineHeight is private, so desired_line_height cannot be forced
    // here. Recalculate from content instead. (Firestorm sets mLineHeight to
    // mDesiredLineHeight when it is > -1.)
    updateLineHeight();
    updateLayout();
}
