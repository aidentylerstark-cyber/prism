 ...
 *
 * Adapted for Prism from the Firestorm Viewer sources. Column init params
 * are tracked in this subclass because the Prism base LLScrollListCtrl does
 * not carry the Firestorm scroll list extensions.
 * $/LicenseInfo$
 */
...
public:
    ...
    // Prism: kept in this subclass; the base viewer's LLScrollListCtrl does not
    // store the column params it was created with, but the radar needs them to
    // re-create columns when toggling column visibility.
